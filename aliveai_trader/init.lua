	...
	on_spawn=function(self)
			...
			for i=0,20,1 do
				for i, v in pairs(minetest.registered_items) do
					if math.random(1,10)==1 and not self.save_inventory[i] and minetest.get_item_group(i, "not_in_creative_inventory")==0 and minetest.get_all_craft_recipes(i) then
						count=count+1
						self.save_inventory[i]=1
			...
			for i=0,20,1 do
				for i, v in pairs(minetest.registered_items) do
					if math.random(1,20)==1 and not self.save_inventory[i] and minetest.get_item_group(i, "not_in_creative_inventory")==0 and minetest.get_all_craft_recipes(i) then
						count=count+1
						c=math.random(5,20)
...
aliveai_trader.form=function(self,player)
	...
	for i, v in pairs(self.save_inventory) do
		c=c+1
	...
	.. but
	.."label[0,2;Pay with:]"
	.."label[" .. (self.save_pos-1.2) ..",2.5;(Selected)]"
	.. but2
			...
			if pressed["buy" .. ii] then
				...
				for i, v in pairs(self.save_inventory) do
					...
					if c==ii then
						...
						for iii, vv in pairs(self.save) do
							cc=cc+1
							if cc==self.save_pos then
