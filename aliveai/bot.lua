...
--mission create
	if aliveai.enable_build==true and self.building==1 and self.mission=="" and not self.home then			--setting up for build a home
...
aliveai.create_bot=function(def)
	...
	if def.texture and type(def.texture)=="table" and type(def.texture[1])=="string" then itemtexture=def.texture[1] end
...
on_activate=function(self, staticdata)
		if staticdata~="" then
			...
			if r.save_inventory then self.save_inventory=r.save_inventory end
			if r.save_pos then self.save_pos=r.save_pos end
			if r.save then self.save=r.save end
...
get_staticdata = function(self)
		...
		if self.save_inventory then r.save_inventory=self.save_inventory end
		if self.save_pos then r.save_pos=self.save_pos end
		if self.save then r.save=self.save end
